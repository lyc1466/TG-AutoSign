...
- ?? / Added: ???????????????????????????????????????????????????? 5 ???????????????????????????????????????????????????????????????? / Submit manual sign-task runs to a background runner so the API returns immediately, duplicate same-account task triggers get a Chinese prompt, and a status endpoint supports real-time progress.
