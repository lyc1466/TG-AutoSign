...
- 2026-04-24 Telegram 完成通知配置改为完全由 UI 管理，不依赖新增 Docker 环境变量
- 2026-04-24 增加签到任务消息监控与历史回看一期能力，并支持将消息历史保留设为 0 禁用
- 2026-04-24 扩展签到任务历史 JSON 的 `message_events` 字段，并保持旧记录兼容读取
- 2026-04-24 增加 `SIGN_TASK_HISTORY_MAX_MESSAGE_EVENTS` 运行时配置，支持限制或关闭消息事件历史保留
